...
export class ControllerManager {
     ...
     * @param args 
     */
    public async ApplyEvent(controllerType: ControllerType, eventName: string, ...args: any): Promise<any> {
        if (!this._controllers.has(controllerType)) {
            console.warn(`控制器{${controllerType}}没有被注册`);
            return;
        }
       return await this._controllers.get(controllerType)?.ApplyEvent(eventName, ...args);
    }
