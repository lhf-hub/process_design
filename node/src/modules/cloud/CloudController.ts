...
export class CloudController extends BaseController {
    ...
    private async OnCreateNode(...args: any): Promise<void> {
        ...
        try {
            ...
            model?.SetAllFieldValue(
                ...
                { key: "size", value: buffer?.length ?? 0 },
    ...
    private async OnRenameNode(...args: any): Promise<void> {
        ...
        try {
            ...
            model?.SetFieldValue("modify_date", new Date(Date.now()).toLocaleString().replace(',', '-'));
    ...
    public async InitProjectStructure(...args: any): Promise<boolean> {
        var result: any;
        var model = this.InstantiateModel<CloudFileModel>(ModelType.CloudFile);
        ...
        try {
            ...
            //     { key: "date", value: new Date(Date.now()).toLocaleString().replace(',', '-') },
            //     { key: "employee_id", value: employee.id },
            //     { key: "project_id", value: requestData?.GetParam("project_id") }
            // )
            // await logger?.Insert();
